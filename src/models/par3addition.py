...
R_J = lambda kvals, J, A, P, J_cyto_r, t, x_i: kvals["k_onJ"] * J_cyto_r - kvals["k_JP"]*(P[x_i]**kvals["gamma"])*J[x_i]
R_A = lambda kvals, J, A, P, A_cyto_r, t, x_i: -kvals["k_offA"]*A[x_i] \
    + kvals["k_AJ"] * (J[x_i] ** kvals["alpha"]) * A_cyto_r
R_P = lambda kvals, J, A, P, P_cyto_r, t, x_i: kvals["k_onP"] * P_cyto_r \
    - kvals["k_offP"] * P[x_i] - kvals["k_PA"] * (A[x_i] ** kvals["beta"]) * P[x_i]


def odefunc(t, U, kvals):
    assert len(U) == 3 * kvals["Nx"]
    ...
    dudt_J[kvals["Nx"]-1] = kvals["D_J"] * disc_diffusion_term(kvals, J, kvals["Nx"]-1) \
        ...
        + R_J(kvals, J, A, P, J_cyto_r, t, kvals["Nx"]-1)
    dudt_A[kvals["Nx"]-1] = kvals["D_A"] * disc_diffusion_term(kvals, A, kvals["Nx"]-1) \
        + R_A(kvals, J, A, P, A_cyto_r, t, kvals["Nx"]-1)
    dudt_P[kvals["Nx"]-1] = kvals["D_P"] * disc_diffusion_term(kvals, P, kvals["Nx"]-1) \
        ...
        + R_P(kvals, J, A, P, P_cyto_r, t, kvals["Nx"]-1)
    ...
    for x_i in np.arange(0, kvals["Nx"] - 1):
        dudt_J[x_i] = kvals["D_J"] * disc_diffusion_term(kvals, J, x_i) \
              ...
              + R_J(kvals, J, A, P, J_cyto_r, t, x_i)
        dudt_A[x_i] = kvals["D_A"] * disc_diffusion_term(kvals, A, x_i) \
              + R_A(kvals, J, A, P, A_cyto_r, t, x_i)
        dudt_P[x_i] = kvals["D_P"] * disc_diffusion_term(kvals, P, x_i) \
              ...
              + R_P(kvals, J, A, P, P_cyto_r, t, x_i)

    return dudt_J + dudt_A + dudt_P
...
def run_model(args=None):
    ...
    kvals["initial_condition"] = kvals["initial_condition"] if "initial_condition" in kvals else [0]*(kvals["Nx"]*3)
...
def animate_plot(sol, kvals: dict, save_file=False, file_code: str = None, rescale=False):
    ...
    line3, = ax.plot(kvals["X"], sol.y[2*kvals["Nx"]:, 0]/scalar, label="posterior", color="orange")
    ...
    def animate(t_i):
        ...
        line3.set_ydata(sol.y[2*kvals["Nx"]:, t_i]/scalar)
